...
| EXT-01   | 替代／例外 | 已有活動中心 10:00～12:00；新申請同日 11:00～13:00            | 呼叫 create(request)                              | VenueBooking 回傳衝突；Controller 不建立資料                                 | 維持建立中／無新紀錄      | 回傳 created=false，申請總數仍為 2                | 通過       | `ApplicationServiceTest` 的 conflict 情境                        |
...
| IST-01   | 非法轉移   | 一筆已核准申請                                                | 呼叫 updateStatus(id, "待補件", "核准後不應補件") | ActivityApplication 應拒絕，集合與狀態不變                                   | 已核准                    | 暫時狀態探針實際顯示「已核准申請被改為待補件」    | 失敗       | 暫時探針輸出`IST-01=FAIL`；`Main.java` 233～268 未檢查目前狀態 |
...
```text
目前看到的症狀：
IST-01 失敗；ActivityApplication 已核准後，Java 後端仍接受 updateStatus(id, "待補件", note)，狀態被非法回轉。
...
下一個可驗證的修正步驟：
先在 ApplicationServiceTest 加入「待審核→已核准→嘗試待補件」失敗測試，再依 Q 任務書讓 ActivityApplication／狀態規則拒絕非待審核的審核更新。
...
重測結果：
尚待 OO-01 實作修正後重測；目前基準為既有測試全數通過、IST-01 失敗、IST-02 通過。
